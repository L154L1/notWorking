...
public class BinaryTree<Node extends BinaryTree.BTNode<Node>> {
	...
	public int totalDepth() {
		...
		while (u != nil) {
			if (prev == u.parent) {
				d += c;	// first time seeing this node
				if (u.left != nil) {
					c++;
					next = u.left;
				} else if (u.right != nil) {
		...
		while (u != nil) {
			if (prev == u.parent) {
				if (u.left != nil) {
					...
					next = u.right;
				}	else {// if no left or right child is a leaf
					d += depth(u);
					next = u.parent;
				}
			} else if (prev == u.left) {
	...
	public String bracketSequence() {
		...
		Node u = r, prev = nil, next;
		while (u != nil) {
			if (prev == u.parent) {// coming from up top, to right or left
				sb.append("(");
				if (u.left != nil) {// if there is a left child
					next = u.left;
				} else if (u.right != nil) {// if there is a right child - means there was no left child if hit this condition ever
					sb.append(".");
					next = u.right;
				}	else {// if no left or right child is a leaf
					sb.append("..");
			...
			} else if (prev == u.left) {// coming from left below to up
				sb.append(")");
				if (u.right != nil) {
					next = u.right;
				} else {
					sb.append("."); // if no right child
					next = u.parent;
				}
			} else {// if coming from right to up
				next = u.parent;
				sb.append(")");
			}
			prev = u;
	...
	public static void main(String[] args) {
		BinaryTree<EndNode> t = randomBST(10);
		System.out.println(t);
		System.out.println(t.totalDepth());
		System.out.println(t.totalDepthSlow());
		System.out.println(t.totalLeafDepth());
